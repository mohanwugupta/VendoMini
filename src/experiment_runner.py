...
from src.logging_utils import log_step, save_summary, setup_logging
...
class ExperimentRunner:
    ...
    def __init__(self, config, cluster_mode: bool = False):
        ...
        self.checkpoints_dir = self.project_root / self.config.get(
    ...
    def run_single_experiment(
        self, params: Dict[str, Any], seed: int = 42
    ) -> Dict[str, Any]:
        ...
        model_name = (
            env_config.get("agent", {})
            .get("model", {})
            .get("name", env_config.get("model", {}).get("name", "unknown"))
        )
        ...
        try:
            ...
            if agent.client is None:
                ...
                print("[ERROR] Experiment cannot run without a working LLM - aborting")
        ...
        print("[*] Initializing PE calculator and crash detector...")
        ...
        for step in range(max_steps):
            try:
                ...
                print(
                    f"[DEBUG] Step {step}: Observation received (day={observation.get('day', 0)}, budget=${observation.get('budget', 0):.2f})"
                )

                # Agent makes decision
                print(
                    f"[DEBUG] Step {step}: Calling agent.get_action_and_prediction()..."
                )
                action_start = time.time()
                action, prediction = agent.get_action_and_prediction(
                    observation, available_tools
                )
                action_time = time.time() - action_start
                print(
                    f"[DEBUG] Step {step}: Agent decision received ({action_time:.2f}s, {action_time/60:.1f} min)"
                )
                ...
                result["message"] = (
                    f"[{tool_name}] returned: {json.dumps(action_result)}"
                )
                ...
                # Check for crashes
                crashed, crash_type = crash_detector.update(
                    step_data[
                        -self.base_config.get_crash_config().get("window_size", 10) :
                    ]
                    if step_data
                    else []
                )
                ...
                if crashed and crash_detector.should_terminate(step):
                    print(
                        f"[*] Terminating at step {step}: {crash_type} (crash detected at step {crash_detector.crash_step})"
                    )
                    break
                elif crashed:
                    ...
                    if step == crash_detector.crash_step:
                        print(
                            f"[*] Crash detected at step {step}: {crash_type} (continuing for {crash_detector.continue_after_crash} more steps)"
                        )
                ...
                if env_done:
                    reason = (
                        "max_days"
                        if env.current_day >= env.max_steps
                        else (
                            "budget_depleted"
                            if env.budget < -100
                            else (
                                "max_actions"
                                if env.action_count >= env.max_actions
                                else "customer_order_failures"
                            )
                        )
                    )
        ...
        summary = {
            ...
            "scratchpad_final_size": len(
                env.scratchpad
            ),  # Add size for quick reference
...
def main():
    ...
    parser.add_argument(
        "--n-jobs", type=int, default=1, help="Number of parallel jobs (local mode)"
    )
    parser.add_argument(
        "--cluster", action="store_true", help="Run in cluster mode (SLURM)"
    )
