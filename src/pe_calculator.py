...
class PECalculator:
    ...
    def __init__(self, windows: list = None):
        ...
        self.alphas = {"fast": 0.3, "med": 0.1, "slow": 0.01}
    ...
    def compute_pe(
        self, prediction_card: Optional[Dict[str, Any]], actual_outcome: Dict[str, Any]
    ) -> Dict[str, float]:
        """
        ...
        if prediction_card is None:
            ...
            return {"temporal": 0.0, "quantity": 0.0, "cost": 0.0, "causal": 0.0}

        pes = {}

        # Temporal PE: |pred_day - actual_day| / max(pred_day, 1)
        if (
            "expected_delivery_day" in prediction_card
            and "actual_delivery_day" in actual_outcome
        ):
        ...
        if (
            "expected_quantity" in prediction_card
            and "actual_quantity" in actual_outcome
        ):
    ...
    def get_windowed_stats(self) -> Dict[str, Dict[str, float]]:
        ...
        for pe_type in ["temporal", "quantity", "cost", "causal"]:
            ...
            stats[pe_type] = {
                ...
                "max": np.max(values),
                "recent_mean_10": (
                    np.mean(values[-10:]) if len(values) >= 10 else np.mean(values)
                ),
                "recent_mean_100": (
