...
def setup_cluster_paths(
    base_dir: Optional[str] = None, models_dir: Optional[str] = None
) -> Dict[str, str]:
    """
...
def expand_grid_slurm(config: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    grid_values = [
        grid[k] if isinstance(grid[k], list) else [grid[k]] for k in grid_keys
    ]
    ...
    for combo_idx, combo in enumerate(combinations):
        for rep in range(replications):
            ...
            params["seed"] = (
                base_seed + combo_idx * replications + rep
                if base_seed
                else combo_idx * replications + rep
            )
            params_list.append(params)
...
def get_task_params_slurm(config: Dict[str, Any], task_id: int) -> Dict[str, Any]:
    ...
    if task_id >= len(all_params):
        raise ValueError(
            f"Task ID {task_id} exceeds number of parameter combinations ({len(all_params)})"
        )
...
def save_task_results(
    task_id: int, results: Dict[str, Any], output_dir: str, prefix: str = "task"
):
    """
...
def aggregate_task_results(
    input_dir: str, output_file: str, prefix: str = "task"
) -> List[Dict[str, Any]]:
    """
...
def get_local_model_path(models_dir: str, model_name: str) -> Optional[str]:
    ...
    if os.path.exists(base_model_dir):
        ...
        has_required = all(
            os.path.exists(os.path.join(base_model_dir, f)) for f in required_files
        )
