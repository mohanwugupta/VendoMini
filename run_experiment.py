...
def main():
    ...
    parser.add_argument(
        "--cluster", action="store_true", help="Run in cluster mode (SLURM array job)"
    )
    parser.add_argument(
        "--task-id",
        type=int,
        default=None,
        help="Task ID (overrides SLURM_ARRAY_TASK_ID)",
    )
    ...
    if args.cluster or env_info["is_slurm"]:
        ...
        print(
            f"Crashed: {crashed_count}/{len(results)} ({100*crashed_count/len(results):.1f}%)"
        )
