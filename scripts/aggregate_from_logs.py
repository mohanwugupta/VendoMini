...
def extract_detailed_metrics(run_dir: Path, summary: dict) -> dict:
    ...
    for step in steps:
        ...
        if "day" in state and "budget" in state:
            budget_trajectory.append({"day": state["day"], "budget": state["budget"]})
    ...
    metrics = {
        ...
        "most_common_action": (
            max(action_counts.items(), key=lambda x: x[1])[0] if action_counts else None
        ),
        ...
        "pe_causal_fast_final": (
            final_cumulative_pe.get("causal", {}).get("fast", 0)
            if final_cumulative_pe
            else 0
        ),
        "pe_causal_med_final": (
            final_cumulative_pe.get("causal", {}).get("med", 0)
            if final_cumulative_pe
            else 0
        ),
        "pe_causal_slow_final": (
            final_cumulative_pe.get("causal", {}).get("slow", 0)
            if final_cumulative_pe
            else 0
        ),
        # Budget metrics
        ...
        "budget_min": (
            min([b["budget"] for b in budget_trajectory]) if budget_trajectory else 0
        ),
        "budget_max": (
            max([b["budget"] for b in budget_trajectory]) if budget_trajectory else 0
        ),
        "went_bankrupt": (
            budget_trajectory[-1]["budget"] <= 0 if budget_trajectory else False
        ),
    }
    ...
    if len(budget_trajectory) >= 2:
        ...
        if total_days > 0:
            budget_change = (
                budget_trajectory[-1]["budget"] - budget_trajectory[0]["budget"]
            )
...
def main():
    ...
    print(
        f"Survived: {len(all_results) - crashed_count} ({100*(len(all_results)-crashed_count)/len(all_results):.1f}%)"
    )
    ...
    if crash_types:
        ...
        for crash_type, count in sorted(
            crash_types.items(), key=lambda x: x[1], reverse=True
        ):
            print(f"  {crash_type}: {count} ({100*count/crashed_count:.1f}%)")

    # Time to crash stats
    times_to_crash = [
        r.get("total_steps", 0) for r in all_results if r.get("crashed", False)
    ]
    if times_to_crash:
        print("\nTime to crash (steps):")
    ...
    if scratchpad_users:
        print("\nScratchpad usage:")
        print(
            f"  Used scratchpad: {len(scratchpad_users)}/{len(all_results)} ({100*len(scratchpad_users)/len(all_results):.1f}%)"
        )
        ...
        print(
            f"  Read/Write ratio: {total_reads/total_writes:.2f}"
            if total_writes > 0
            else "  Read/Write ratio: N/A"
        )
    ...
    if action_diversity:
        print(
            f"  Action diversity (mean): {np.mean(action_diversity):.1f} unique tools"
        )
    ...
    if went_bankrupt > 0:
        print("\nBankruptcy:")
        print(
            f"  Went bankrupt: {went_bankrupt}/{len(all_results)} ({100*went_bankrupt/len(all_results):.1f}%)"
        )

    burn_rates = [
        r.get("budget_burn_rate", 0) for r in all_results if "budget_burn_rate" in r
    ]
    if burn_rates:
