        ...
        for log_entry in logs:
            ...
            if event in ["money_balance", "net_worth", "units_sold"]:
                timeseries_events.append(
                    {"timestamp": timestamp, "event_type": event, "value": payload}
                )
            elif event == "tool_calls":
                tool_events.append({"timestamp": timestamp, "tool_name": payload})
...
def main():
    parser = argparse.ArgumentParser(
        description="Extract time-series data from JSON log files"
    )
    parser.add_argument(
        "--input", "-i", default="data", help="Input directory containing model folders"
    )
    parser.add_argument(
        "--output",
        "-o",
        default="processed/timeseries_data.json",
            ...
            for json_file in model_folder.glob("*.json"):
                ...
                if json_file.name.endswith(".json"):
                    try:
                        ...
                        print(
                            f"    Could not extract epoch from filename: {json_file.name}"
                        )

                # Extract run ID from filename
                run_id = extract_run_id_from_filename(json_file.name)

                metadata, timeseries_events, tool_events = extract_timeseries_from_json(
                    json_file
                )
                if metadata:
                    run_data = {
