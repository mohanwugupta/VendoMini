...
def categorize_goal_action(tool_calls, message_content):
    ...
    if "get_money_balance" in tools_str or any(
        word in content_lower for word in ["balance", "money", "funds", "budget"]
    ):
    ...
    if any(
        ...
        for word in ["price", "pricing", "cost", "sell", "sales", "revenue"]
    ):
        return "pricing_strategy"

    # Market research actions
    if "ai_web_search" in tools_str or any(
        word in content_lower for word in ["research", "market", "competitor", "demand"]
    ):
        return "market_research"

    # Customer communication actions
    if any(
        tool in tools_str for tool in ["read_email", "send_email", "read_email_inbox"]
    ):
        return "customer_communication"

    # Planning/strategy actions
    if any(
        tool in tools_str for tool in ["write_scratchpad", "read_scratchpad"]
    ) or any(
        word in content_lower for word in ["plan", "strategy", "goal", "objective"]
    ):
    ...
    if any(
        word in content_lower
        for word in ["analyze", "analysis", "evaluate", "assess", "performance"]
    ):
        return "analysis"
...
def extract_conversation_data(data, epoch, goal_info):
    ...
    for log in logs:
        ...
        current_step["relative_timestamp"] = max(
            current_step["relative_timestamp"], timestamp
        )
        ...
        # Track tool calls - this indicates a new step
            ...
            if len(current_step["tool_calls"]) > 1 or len(steps) > 0:
                ...
                current_step["context_window_position"] = (
                    estimate_context_window_position(
                ...
                current_step = {
                    ...
                    "context_window_position": estimate_context_window_position(
        ...
        # Track message content
            ...
            if payload:  # Only if there's actual content
                ...
                if msg_has_goal:
                    current_step["message_goal_count"] = (
                        current_step.get("message_goal_count", 0) + 1
                    )
...
def has_goal_content(message_content, goal_info):
    ...
    if goal_info["primary_goal"]:
        ...
        if matches >= min(
            2, len(goal_words)
        ):  # At least 2 words or all words if less than 2
            goal_match = True
    ...
    try:
        ...
        for step_data in conversation_data:
            goal_record = {
                ...
                "goal_keywords": (
                    "; ".join(goal_info["goal_keywords"])
                    if goal_info["goal_keywords"]
                    else None
                ),
                "goal_metrics": (
                    "; ".join(goal_info["goal_metrics"])
                    if goal_info["goal_metrics"]
                    else None
                ),
                "success_criteria": (
                    "; ".join(goal_info["success_criteria"])
                    if goal_info["success_criteria"]
                    else None
                ),
                "constraints": (
                    "; ".join(goal_info["constraints"])
                    if goal_info["constraints"]
                    else None
                ),
                "strategies": (
                    "; ".join(goal_info["strategies"])
                    if goal_info["strategies"]
                    else None
                ),
                ...
                "tool_calls_at_step": (
                    "; ".join(step_data["tool_calls"])
                    if step_data["tool_calls"]
                    else None
                ),
                "message_content": (
                    step_data["message_content"].strip()
                    if step_data["message_content"]
                    else None
                ),
                "message_content_length": len(step_data["message_content"]),
                "message_goal_count": step_data.get("message_goal_count", 0),
                "has_goal_content": has_goal_content(
                    step_data["message_content"], goal_info
                ),
                "goal_action_type": categorize_goal_action(
                    step_data["tool_calls"], step_data["message_content"]
                ),
            }
...
def main():
    parser = argparse.ArgumentParser(
        description="Extract model goal data from JSON files"
    )
    parser.add_argument(
        "--input",
        "-i",
        default="data",
        help="Input directory containing model folders with JSON files",
    )
    parser.add_argument(
        "--output", "-o", default="processed", help="Output directory for CSV files"
    )
    ...
    for model_folder in input_dir.iterdir():
        if (
            ...
            and model_folder.name != "processed"
        ):
    ...
    if all_goal_data:
        ...
        unique_goals = len(
            set(row["primary_goal"] for row in all_goal_data if row["primary_goal"])
        )
        ...
        goal_content_count = sum(
            1 for row in all_goal_data if row["has_goal_content"] == 1
        )
