...
try:
    ...
    print(
        "Warning: vaderSentiment not available. Install with: pip install vaderSentiment"
    )
...
class SentimentBreakdownDetector:
    ...
    def extract_linguistic_features(self, text: str) -> Dict[str, float]:
        ...
        features["caps_ratio"] = sum(1 for c in text if c.isupper()) / max(
            char_count, 1
        )
    ...
    def calculate_breakdown_score(self, features: Dict[str, float]) -> float:
        ...
        if "exclamation_ratio" in features:
            score += (
                min(features["exclamation_ratio"] * 50, 1) * linguistic_weight * 0.2
            )
    ...
    def analyze_message(
        self, content: str, role: str, step: int, epoch: int
    ) -> Dict[str, float]:
...
def main():
    parser = argparse.ArgumentParser(
        description="Detect model breakdowns via sentiment"
    )
    parser.add_argument(
        "--results",
        type=str,
    ...
    parser.add_argument(
        "--output",
        type=str,
        default="processed/sentiment_analysis.csv",
