        ...
        for metric in metrics_to_extract:
            ...
            if values:
                ...
                for belief in beliefs:
                    if (
                        belief["epoch"] == epoch
                        and belief["step"] == step
                        and belief["name"] == metric
                        and belief["metric_type"] == "individual"
                        and belief["summary_value"] is None
                    ):
...
def extract_model_reasoning_patterns(messages):
    ...
    for i, msg in enumerate(messages):
        ...
        tracks_metrics = bool(
            re.search(r"(?:track|monitor|keep track|record|note)", content_lower)
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Extract model beliefs from JSON log files"
    )
    parser.add_argument(
        "--input",
        "-i",
    ...
    if all_beliefs:
        ...
        if "name" in beliefs_df.columns:
            print("- Belief metrics extracted:")
        ...
        if not summary_df.empty:
            ...
            for metric in summary_df["name"].unique():
                ...
                print(
                    f"  {metric}: mean={metric_data.mean():.2f}, std={metric_data.std():.2f}, "
    ...
    if all_beliefs:
        ...
        print(
            f"Summary file contains {len(summary_only_df)} records (one per metric mention)"
        )
