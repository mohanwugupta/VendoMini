...
# Import adapter
try:
    from steps_adapter import extract_assistant_text, iter_steps_jsonl
except ImportError:
...
def process_single_file(steps_jsonl, run_id):
    ...
    for s in iter_steps_jsonl(steps_jsonl):
        ...
        total_storage = (
            sum(storage_dict.values()) if isinstance(storage_dict, dict) else 0
        )
        ...
        rows.append(
            {
                ...
                "cum_temporal_fast": safe_get(
                    s, "cumulative_pe", "temporal", "fast", default=0.0
                ),
                "cum_quantity_fast": safe_get(
                    s, "cumulative_pe", "quantity", "fast", default=0.0
                ),
                "cum_cost_fast": safe_get(
                    s, "cumulative_pe", "cost", "fast", default=0.0
                ),
                "cum_causal_fast": safe_get(
                    s, "cumulative_pe", "causal", "fast", default=0.0
                ),
...
def main():
    ...
    parser.add_argument(
        "--logs-dir", type=str, help="Path to logs directory", default="logs"
    )
    parser.add_argument(
        "--output",
        type=str,
        default="processed/steps_timeseries.csv",
