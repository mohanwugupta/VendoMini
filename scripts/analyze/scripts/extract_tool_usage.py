            ...
            for i, tool_call in enumerate(tool_calls):
                if isinstance(tool_call, dict):
                    ...
                    tool_event = {
                        ...
                        "tool_name": tool_name,
                        "tool_category": (
                            categorize_tool(tool_name) if is_valid else "hallucinated"
                        ),
                        "is_valid": is_valid,
...
def extract_tool_usage_from_events(
    events: List[Dict],
        ...
        # Look for other tool-related events
            ...
            if payload:
                tool_event = {
                    ...
                    "event_index": i,
                    "timestamp": event.get(
                        "timestamp", event.get("relative_timestamp", i)
                    ),
                            ...
                            for tool_call in tool_calls:
                                if isinstance(tool_call, dict):
                                    ...
                                    tool_event = {
                                        ...
                                        "timestamp": event.get(
                                            "timestamp",
                                            event.get("relative_timestamp", i),
                                        ),
                                        "tool_name": tool_name,
                                        "tool_category": categorize_tool(tool_name),
        ...
        with zipfile.ZipFile(file_path, "r") as zf:
            ...
            for sample_file in sample_files:
                try:
                    ...
                    message_tools, valid_message_counts, hallucinated_message_counts = (
                        extract_tool_usage_from_messages(messages)
                    )
                    ...
                    # Calculate totals
                    total_valid_tools = sum(valid_message_counts.values()) + sum(
                        event_counts.values()
                    )
                    ...
                    hallucination_rate = (
                        (total_hallucinated_tools / total_tools)
                        if total_tools > 0
                        else 0
                    )
                    ...
                    epoch_record = {
                        **metadata,
                        ...
                        "tools_from_messages_hallucinated": sum(
                            hallucinated_message_counts.values()
                        ),
                    ...
                    if hallucinated_message_counts:
                        most_hallucinated = Counter(
                            hallucinated_message_counts
                        ).most_common(1)[0]
                        epoch_record["most_hallucinated_tool"] = most_hallucinated[0]
                        epoch_record["most_hallucinated_tool_count"] = (
                            most_hallucinated[1]
                        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Extract tool usage data from .eval files"
    )
    parser.add_argument(
        "--input", "-i", default="data", help="Input directory containing .eval files"
    )
    parser.add_argument(
        "--output",
        "-o",
        default="processed/tool_usage_epochs.csv",
        help="Output CSV file",
    )
    parser.add_argument(
        "--output-detailed",
        default="processed/tool_usage_detailed.csv",
        help="Output CSV file for detailed tool events",
    )
    ...
    if all_epoch_data:
        ...
        total_tools = sum(
            record.get("total_tools_used", 0) for record in all_epoch_data
        )
        total_valid_tools = sum(
            record.get("total_valid_tools", 0) for record in all_epoch_data
        )
        total_hallucinated_tools = sum(
            record.get("total_hallucinated_tools", 0) for record in all_epoch_data
        )
        avg_tools_per_epoch = total_tools / len(all_epoch_data) if all_epoch_data else 0
        overall_hallucination_rate = (
            (total_hallucinated_tools / total_tools) if total_tools > 0 else 0
        )
        ...
        all_tool_fields = [
            ...
            if field.startswith("tool_") and not field.startswith("tool_category")
        ]
        ...
        print("- Most used valid tools overall:")
        for tool, count in sorted(
            tool_totals.items(), key=lambda x: x[1], reverse=True
        )[:10]:
            print(f"  {tool}: {count}")

        # Find most common hallucinated tools
        hallucinated_tool_fields = [
            field for field in fieldnames if field.startswith("hallucinated_")
        ]
        ...
        if hallucinated_totals:
            print("- Most common hallucinated tools:")
            for tool, count in sorted(
                hallucinated_totals.items(), key=lambda x: x[1], reverse=True
            )[:10]:
                print(f"  {tool}: {count}")
        else:
            print("- No hallucinated tools found!")

        # Category analysis
        category_fields = [
            field for field in fieldnames if field.startswith("category_")
        ]
