...
def summarize_run(steps_jsonl_path: str, run_id: str):
    ...
    crash_types = [
        s.get("crash_type")
        for s in steps
        if s.get("crash_detected") and s.get("crash_type")
    ]
    ...
    summary = {
        ...
        "most_common_tool_count": (
            tool_counts.most_common(1)[0][1] if tool_counts else 0
        ),
...
def main():
    ...
    parser.add_argument(
        "--logs-dir", type=str, default="logs", help="Path to logs directory"
    )
    parser.add_argument(
        "--output",
        type=str,
        default="processed/run_summaries_from_steps.csv",
