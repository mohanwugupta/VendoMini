    ...
    try:
        with zipfile.ZipFile(file_path, "r") as zf:
            ...
            metadata = {
                ...
                "completed_samples": header_data.get("results", {}).get(
                    "completed_samples", 0
                ),
            ...
            if "started_at" in stats and "completed_at" in stats:
                ...
                try:
                    start = datetime.fromisoformat(
                        stats["started_at"].replace("+01-00", "+01:00")
                    )
                    end = datetime.fromisoformat(
                        stats["completed_at"].replace("+01-00", "+01:00")
                    )
        ...
        with zipfile.ZipFile(file_path, "r") as zf:
            ...
            for score_entry in results.get("scores", []):
                ...
                if "bool_score" in metrics:
                    scores[f"{score_name}_bool"] = metrics["bool_score"].get(
                        "value", None
                    )
                elif "num_score" in metrics:
                    scores[f"{score_name}_num"] = metrics["num_score"].get(
                        "value", None
                    )
            ...
            for sample_file in sample_files:
                try:
                    ...
                    if "scores" in sample_data:
                        ...
                        if (
                            "boolean_metrics" in scores
                            and "value" in scores["boolean_metrics"]
                        ):
                        ...
                        if (
                            "numeric_metrics" in scores
                            and "value" in scores["numeric_metrics"]
                        ):
                    ...
                    if "store" in sample_data:
                        ...
                        if "net_worth_num" not in epoch_data:
                    ...
                    if "model_usage" in sample_data:
                        ...
                        for model_name, usage in model_usage.items():
                            epoch_data["epoch_input_tokens"] = usage.get(
                                "input_tokens", 0
                            )
                            epoch_data["epoch_output_tokens"] = usage.get(
                                "output_tokens", 0
                            )
                            epoch_data["epoch_total_tokens"] = usage.get(
                                "total_tokens", 0
                            )
                            break  # Just take the first model
...
def main():
    parser = argparse.ArgumentParser(
        description="Extract scoring data from .eval files"
    )
    parser.add_argument(
        "--input", "-i", default="data", help="Input directory containing .eval files"
    )
    parser.add_argument(
        "--output", "-o", default="processed/scoring_data.csv", help="Output CSV file"
    )
    parser.add_argument(
        "--output-samples",
        default="processed/scoring_data_samples.csv",
        help="Output CSV file for per-sample data",
    )
    parser.add_argument(
        "--output-epochs",
        default="processed/scoring_data_epochs.csv",
        help="Output CSV file for per-epoch data",
    )
    ...
    if all_runs:
        ...
        if all_epoch_data:
            ...
            score_fields = [
                k
                for k in example_epoch.keys()
                if k.endswith("_bool") or k.endswith("_num")
            ]
            if score_fields:
                print(
                    f"- Available metrics per epoch: {', '.join(sorted(score_fields))}"
                )
