        ...
        with zipfile.ZipFile(eval_file_path, "r") as zf:
            ...
            for pattern in additional_patterns:
                ...
                for match in matches:
                    if len(match) == 2:
                        identity_info["additional_details"].append(
                            f"{match[0]}: {match[1]}"
                        )
    ...
    try:
        ...
        for step_data in conversation_data:
            position_record = {
                ...
                "identity_additional_details": (
                    "; ".join(identity_info["additional_details"])
                    if identity_info["additional_details"]
                    else None
                ),
                ...
                "has_identity_content": has_identity_content(
                    step_data["message_content"], identity_info
                ),
            }
...
def has_identity_content(message_content, identity_info):
    ...
    if identity_info["name"]:
        ...
        for part in name_parts:
            if (
                len(part) >= 3 and part.lower() in message_lower
            ):  # Only check names 3+ chars
                name_match = True
                break
...
def main():
    parser = argparse.ArgumentParser(
        description="Extract model identity position data from JSON files"
    )
    parser.add_argument(
        "--input",
        "-i",
    ...
    if all_position_data:
        ...
        unique_occupations = len(
            set(
                row["identity_occupation"]
                for row in all_position_data
                if row["identity_occupation"]
            )
        )
        ...
        identity_content_count = sum(
            1 for row in all_position_data if row["has_identity_content"] == 1
        )
        ...
        print("- Context window positions:")
